...
public class MineRecodeActivity extends BaseMvpActivity {
    ...
    protected void initTitle() {
        titleHeaderTv.setText(ResourcesManager.getString(R.string.mine_recode));
    }
    ...
    protected void initView() {
        if (DataManager.getLoginInfo() != null && DataManager.getLoginInfo().getUser().getIsDealer() == 2) {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.mine_recode_register_recode_civ://委托记录
                ActivityManager.gotoActivity(mContext, OrderRecodeActivity.class);
                break;
            case R.id.mine_recode_transaction_recode_civ://成交记录
                ActivityManager.gotoActivity(mContext, TransactionRecodeActivity.class);
                break;
            case R.id.mine_recharge_currency_recode_civ://充币记录
                ActivityManager.gotoActivity(mContext, CurrencyWithDrawRechargeActivity.class);
                break;
            case R.id.mine_recode_currency_recode_civ://提币记录
                ActivityManager.gotoActivity(mContext, CurrencyWithDrawRecodeActivity.class);
                break;
