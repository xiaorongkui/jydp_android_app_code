...
//                }
            ...
            for (int j = 0; j < buffer.size(); j += 4) {
                ...
                c.drawRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
                        buffer.buffer[j + 3], mRenderPaint);
              /*  c.drawRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2] - (buffer.buffer[j + 2] -
              buffer.buffer[j]) / 3,
                        buffer.buffer[j + 3], mRenderPaint);*/
                if (drawBorder) {
                    c.drawRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
                ...
                if (!dataSet.isStacked()) {
                    ...
                    for (int j = 0; j < valuePoints.length * mAnimator.getPhaseX(); j += 2) {
                        ...
                        drawValue(c, dataSet.getValueFormatter(), val, entry, i, valuePoints[j],
                                valuePoints[j + 1] + (val >= 0 ? posOffset : negOffset), dataSet.getValueTextColor(j
                                        / 2));
                    }
                    ...
                    for (int j = 0; j < (valuePoints.length - 1) * mAnimator.getPhaseX(); j += 2) {
                        ...
                        if (vals == null) {
                            ...
                            drawValue(c, dataSet.getValueFormatter(), entry.getVal(), entry, i, valuePoints[j],
                                    valuePoints[j + 1] + (entry.getVal() >= 0 ? posOffset : negOffset), dataSet
        ...
        for (Highlight high : indices) {
            ...
            for (int dataSetIndex = minDataSetIndex;
                ...
                if (index >= 0
                    ...
                    /*重写高亮*/
                    c.drawLine(mBarRect.centerX(), mViewPortHandler.getContentRect().bottom, mBarRect.centerX(), 0,
                            mHighlightPaint);
